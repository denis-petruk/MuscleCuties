...
public class WorkoutPlanner : IWorkoutPlanner
{
    ...
    public IReadOnlyList<WorkoutItem> BuildWorkoutItems(
        ...
        IReadOnlyCollection<WorkoutLog>? workoutLogs = null)
    {
        ...
        {
            if (!daysByWeekday.TryGetValue(dayOfWeek, out var scheduledDays))
            {
                items.Add(BuildRestDayItem(dayOfWeek));
                continue;
            }

            var activeDays = scheduledDays
                .Where(day => day.WorkoutType != WorkoutType.Rest)
                .ToList();
            var visibleDays = activeDays.Count > 0 ? activeDays : [scheduledDays.First()];
            foreach (var day in visibleDays)
            {
                ...
                {
                    items.Add(BuildRestDayItem(day.DayOfWeek, day.Id, latestLog));
                    continue;
                }
                ...
                items.Add(new WorkoutItem
                {
                    ...
                    Title = day.Name,
                    Duration = BuildDurationText(day),
                    ExerciseCountText = BuildExerciseCountText(day),
                    ActivityCountText = BuildActivityCountText(day),
                    DetailsText = BuildDetailsText(day),
                    ...
                    SessionProgressText = BuildSessionProgressText(latestLog),
                    IsCompleted = IsWorkoutCompleted(latestLog)
                });
            }
        }
        ...
        DateTime date)
    {
        ...
        var activitySections = BuildActivitySections(todayWorkout);
        return new TodaysWorkoutSummary(
            todayWorkout.Name,
        ...
        DateTime date)
    {
        ...
        return workoutDays.FirstOrDefault(day => day.DayOfWeek == dayOfWeek && day.WorkoutType != WorkoutType.Rest) ??
               workoutDays.FirstOrDefault(day => day.DayOfWeek == dayOfWeek);
    }
    ...
    {
        return completedLog?.CompletionPercent >= 100;
    }

    private static WorkoutItem BuildRestDayItem(
        int dayOfWeek,
        int workoutDayId = 0,
        WorkoutLog? latestLog = null)
    {
        ...
        return new WorkoutItem
        {
            ...
            ActivityCountText = "0 activities",
            DetailsText = "Rest, recover, enjoy the day.",
            ActivityBackground = WorkoutActivityClassifier.GetBackground(activityTag),
            ActivityTextColor = WorkoutActivityClassifier.GetTextColor(activityTag),
            IsRestDay = true,
            SessionProgressText = BuildSessionProgressText(latestLog),
    ...
    {
        return terms.Any(term => day.Name.Contains(term, StringComparison.OrdinalIgnoreCase)) ||
               day.WorkoutDayExercises.Any(exercise =>
                   terms.Any(term =>
                       exercise.Exercise?.Name.Contains(term, StringComparison.OrdinalIgnoreCase) == true));
    }

    private static IReadOnlyList<WorkoutActivitySection> BuildActivitySections(WorkoutDay day)
    {
    ...
    {
        return WorkoutActivityClassifier.BuildActivityTags(day).Count(tag => tag != WorkoutActivityClassifier.RestTag);
    }
