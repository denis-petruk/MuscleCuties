...
public class WorkoutService : IWorkoutService
{
        ...
        DateTime date)
    {
        ...
        var alreadyLoggedIds = currentLog?.ExerciseLogs
            .Where(log =>
                exercisesByDayExerciseId.TryGetValue(log.WorkoutDayExerciseId, out var dayExercise) &&
                IsExerciseLogComplete(log, dayExercise, day.WorkoutType))
        ...
        foreach (var input in validExerciseLogs)
        {
            ...
            ValidateExerciseLogInput(input, dayExercise, day.WorkoutType);
        ...
        foreach (var input in validExerciseLogs)
        {
            ...
            workoutLog.ExerciseLogs.Add(new WorkoutExerciseLog
            {
                ...
                CompletedDurationSeconds =
                    input.CompletedDurationSeconds is >= 0 ? input.CompletedDurationSeconds : null,
        ...
        CyclePhase phase)
    {
        ...
        var activePlanDays = activePlan is null
            ? []
            : await _workoutRepository.GetWorkoutDaysByPlanAsync(activePlan.Id);
        if (!_workoutPlanGenerator.ShouldRegenerate(activePlan, activePlanDays, profile, phase))
            return activePlan;
    ...
    private async Task<WorkoutPlan?> BuildAndReplaceGeneratedPlanAsync(int userId, CyclePhase phase)
    {
        ...
        var generatedPlan = await _workoutPlanGenerator.GenerateAsync(
            profile,
            phase,
            DateTime.UtcNow);
        ...
        WorkoutExerciseLog? currentLog)
    {
        ...
        return new WorkoutExerciseItem
        {
            ...
            IsLogged = IsExerciseLogComplete(currentLog, dayExercise, workoutType),
            ...
            LoggedRepsText = currentLog is not null ? currentLog.CompletedReps.ToString() :
                dayExercise.Reps > 0 ? dayExercise.Reps.ToString() : string.Empty,
            LoggedWeightText = currentLog?.WeightKg is >= 0f
                ? $"{currentLog.WeightKg.Value:0.#}"
                : BuildSuggestedWeightText(dayExercise, previousLog, metricProfile),
            ...
            LoggedDistanceKmText = currentLog?.DistanceKm is >= 0f
                ? $"{currentLog.DistanceKm.Value:0.#}"
                : BuildSuggestedDistanceText(previousLog, metricProfile),
            LoggedPaceText = currentLog?.PaceSecondsPerKm is >= 0
                ? FormatPace(currentLog.PaceSecondsPerKm.Value)
                : BuildSuggestedPaceText(previousLog, metricProfile),
            LoggedHeartRateText = currentLog?.AverageHeartRateBpm is >= 0
                ? currentLog.AverageHeartRateBpm.Value.ToString()
                : BuildSuggestedHeartRateText(previousLog, metricProfile),
            LoggedPowerWattsText = currentLog?.PowerWatts is >= 0
                ? currentLog.PowerWatts.Value.ToString()
                : BuildSuggestedPowerText(previousLog, metricProfile),
            LoggedCadenceRpmText = currentLog?.CadenceRpm is >= 0
                ? currentLog.CadenceRpm.Value.ToString()
                : BuildSuggestedCadenceText(previousLog, metricProfile),
            LoggedEffortText = currentLog?.EffortRating is >= 1 and <= 10
                ? currentLog.EffortRating.Value.ToString()
                : BuildSuggestedEffortText(previousLog, metricProfile)
        };
    }

    private static bool IsExerciseLogComplete(
        ...
        WorkoutType workoutType)
    {
        ...
        if (metricProfile.UsesEnduranceMetrics)
            return !metricProfile.UsesDuration ||
                   log.CompletedDurationSeconds is > 0;
    ...
    private static void ValidateExerciseLogInput(
        WorkoutExerciseLogInput input,
        WorkoutDayExercise dayExercise,
        WorkoutType workoutType)
    {
        var metricProfile = BuildMetricProfile(dayExercise, workoutType);
        ...
        var exerciseName = dayExercise.Exercise?.Name ?? "this exercise";
        if (input.CompletedSets <= 0 || input.CompletedReps <= 0)
            throw new InvalidOperationException($"Add sets and reps for {exerciseName} before logging it.");

        if (input.WeightKg is null or < 0f)
            throw new InvalidOperationException(
                $"Add kg for {exerciseName} before logging it. Use 0 kg for bodyweight.");
    }
    ...
    private static ExerciseMetricProfile BuildMetricProfile(WorkoutDayExercise exercise, WorkoutType workoutType)
    {
        ...
        return new ExerciseMetricProfile(
            ...
            isCycling || isRunning || isSwimming || isHiit || IsDistanceRecoveryExercise(name),
            isCycling,
            isCycling,
            isCycling || isRunning || isSwimming || isHiit,
            isTimeUnderTension,
            isSwimming);
    }
    ...
    {
        return ContainsAny(name, "plank", "copenhagen");
    }

    private static bool IsCyclingExercise(string name)
    {
        return ContainsAny(name, "bike", "cycle", "cycling", "ride");
    }

    private static bool IsRunningExercise(string name)
    {
        return ContainsAny(name, "run", "jog", "sprint", "tempo");
    }

    private static bool IsSwimmingExercise(string name)
    {
        return ContainsAny(name, "swim", "pool");
    }

    private static bool IsHiitExercise(string name)
    {
        return ContainsAny(name, "hiit", "interval") && !IsCyclingExercise(name) && !IsRunningExercise(name);
    }

    private static bool IsDistanceRecoveryExercise(string name)
    {
        return ContainsAny(name, "walk");
    }
    ...
    private static string BuildExercisePurpose(Exercise? exercise, WorkoutType workoutType)
    {
        ...
        if (ContainsAny(name, "Leg Press", "Leg Extension"))
            return
                "Adds stable quad-focused volume so lower-body sessions can grow without every set becoming a balance drill.";

        if (ContainsAny(name, "Squat", "Lunge", "Step-Up"))
            return
                "Builds usable leg strength through knee and hip control, with enough single-leg work to keep both sides honest.";
        ...
        if (ContainsAny(name, "Romanian Deadlift", "Leg Curl"))
            return
                "Builds hamstrings, glutes, and hinge control so pulling strength progresses without rushing the lower back.";
        ...
        if (workoutType is WorkoutType.Cardio || ContainsAny(name, "Ride", "Walk", "Run", "Jog", "Sprint", "Cycling",
                "Intervals", "Swimming", "HIIT", "Dance"))
    ...
    private static string BuildQuickTips(Exercise? exercise, ExerciseMetricProfile metricProfile)
    {
        ...
        if (ContainsAny(name, "Yoga", "Vinyasa", "Mobility", "Recovery", "Breathing", "Pilates"))
            return
                "Keep effort at a 4 to 6 out of 10. Longer holds are fine; sharp pain is not. Good recovery should make the next session easier.";

        if (ContainsAny(name, "Rock Climbing", "Climb"))
            return
                "Log total climbing time and keep 2 to 3 good attempts in reserve. If grip or elbows feel irritated, switch to easier routes or finish early.";

        if (metricProfile.UsesEnduranceMetrics)
            return
                "Use the same bike, route, or pool setup when you can. Better data comes from repeatable efforts, not random hero days.";

        if (ContainsAny(name, "Squat", "Lunge", "Step-Up", "Romanian Deadlift", "Hip Thrust", "Dumbbell Press",
                "Overhead Press", "Row", "Pulldown"))
            return
                "Aim for 1 to 3 reps in reserve. Add weight next time only when every set hits the target with the same form.";
    ...
    {
        return ContainsAny(day.Name, terms) ||
        ...
        IReadOnlyCollection<WorkoutDayExercise> exercises)
    {
        ...
        if (seconds > 0)
            return
                $"{exerciseCount} {(exerciseCount == 1 ? "exercise" : "exercises")} with {Math.Ceiling(seconds / 60d):0} min";

        if (day.WorkoutType is WorkoutType.Strength && exerciseCount > 0)
            return
                $"{exerciseCount} {(exerciseCount == 1 ? "exercise" : "exercises")}, about {WorkoutDurationEstimator.EstimateStrengthMinutes(exercises)} min";
    ...
    private static string BuildSuggestedDistanceText(
        ...
        ExerciseMetricProfile metricProfile)
    {
        return metricProfile.UsesDistance && previousLog?.DistanceKm is > 0f
    ...
    private static string BuildSuggestedPaceText(
        ...
        ExerciseMetricProfile metricProfile)
    {
        return metricProfile.UsesPace && previousLog?.PaceSecondsPerKm is > 0
    ...
    private static string BuildSuggestedHeartRateText(
        ...
        ExerciseMetricProfile metricProfile)
    {
        return metricProfile.UsesHeartRate && previousLog?.AverageHeartRateBpm is > 0
    ...
    private static string BuildSuggestedPowerText(
        ...
        ExerciseMetricProfile metricProfile)
    {
        return metricProfile.UsesPower && previousLog?.PowerWatts is > 0
    ...
    private static string BuildSuggestedCadenceText(
        ...
        ExerciseMetricProfile metricProfile)
    {
        return metricProfile.UsesCadence && previousLog?.CadenceRpm is > 0
    ...
    private static string BuildSuggestedEffortText(
        ...
        ExerciseMetricProfile metricProfile)
    {
        return metricProfile.UsesEffort && previousLog?.EffortRating is >= 1 and <= 10
    ...
    {
        return $"{paceSecondsPerKm / 60}:{paceSecondsPerKm % 60:00}";
    }
