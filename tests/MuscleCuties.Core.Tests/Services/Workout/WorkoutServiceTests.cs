...
public class WorkoutServiceTests : IClassFixture<DatabaseFixture>
{
        ...
        int workoutDaysPerWeek = 4)
    {
        ...
        await _fixture.Db.SeedWorkoutPlanningDataAsync();
    ...
    public async Task GetPlanSummaryAsync_WithProfileAndNoActivePlan_GeneratesPlan()
    {
        ...
        Assert.Equal("Personalized follicular training", summary.ActivePlan!.Name);
        ...
        Assert.All(summary.WorkoutDays.Where(day => day.WorkoutType != WorkoutType.Rest),
            day => Assert.NotEmpty(day.WorkoutDayExercises));
    }
    ...
    public async Task GetPlanSummaryAsync_WhenGeneratedPlanNoLongerMatchesProfile_ReplacesIt()
    {
        ...
        var activePlanCount =
            await _fixture.Db.WorkoutPlans.CountAsync(plan => plan.UserId == user.Id && plan.IsActive);
    ...
    [Fact]
    public async Task GetPlanSummaryAsync_WithMultipleActivityPreferences_DoesNotRegenerateValidPlan()
    {
        var user = await SeedUserWithProfileAsync("stacked-plan@test.local", workoutDaysPerWeek: 4);
        var profile = await _fixture.Db.UserProfiles.SingleAsync(item => item.UserId == user.Id);
        profile.PreferredWorkoutActivityTypes = WorkoutActivityPreferences.Serialize(
            [WorkoutActivityType.HighVolumeStrength, WorkoutActivityType.RockClimbing]);
        await _fixture.Db.SaveChangesAsync();
        var service = CreateService();

        var first = await service.GetPlanSummaryAsync(user.Id, CyclePhase.Follicular);
        var second = await service.GetPlanSummaryAsync(user.Id, CyclePhase.Follicular);

        Assert.NotNull(first.ActivePlan);
        Assert.Equal(first.ActivePlan!.Id, second.ActivePlan!.Id);
        Assert.Equal(4, first.WorkoutDays
            .Where(day => day.WorkoutType != WorkoutType.Rest)
            .Select(day => day.DayOfWeek)
            .Distinct()
            .Count());
        Assert.All(first.WorkoutDays.Where(day => day.WorkoutType != WorkoutType.Rest),
            day => Assert.NotEmpty(day.WorkoutDayExercises));
    }
    ...
    public async Task GetWorkoutSessionDetailAsync_ForCardio_UsesEnduranceMetrics()
    {
        ...
        Assert.Equal("Last 30 min with 160 W with 88 rpm with 5 km with 145 bpm with effort 6/10",
            exercise.PreviousText);
        Assert.Equal("Start near 160 W, keep cadence around 80-95 rpm, and let heart rate confirm the effort.",
            exercise.RecommendationText);
    ...
    [Fact]
    public async Task LogWorkoutSessionAsync_ForStrengthWithoutWeight_DoesNotSaveAsLogged()
    {
        var user = await SeedUserWithProfileAsync("workout_svc_missing_weight@test.com");
        var service = CreateService();
        var (day, dayExercise) = await SeedWorkoutDayAsync(user.Id, "Hip Thrust", 4, 8);

        var error = await Assert.ThrowsAsync<InvalidOperationException>(() =>
            service.LogWorkoutSessionAsync(
                user.Id,
                day.Id,
                [new WorkoutExerciseLogInput(dayExercise.Id, dayExercise.ExerciseId, 4, 8, null)],
                DateTime.Today));

        Assert.Contains("Add kg", error.Message);
        Assert.False(await _fixture.Db.WorkoutLogs.AnyAsync(log =>
            log.UserId == user.Id &&
            log.WorkoutDayId == day.Id));
    }

    [Fact]
    public async Task GetWorkoutSessionDetailAsync_ForStrengthLogMissingWeight_ShowsExerciseNotLogged()
    {
        var user = await SeedUserWithProfileAsync("workout_svc_missing_weight_detail@test.com");
        var service = CreateService();
        var (day, dayExercise) = await SeedWorkoutDayAsync(user.Id, "Hip Thrust", 4, 8);
        await _fixture.Db.WorkoutLogs.AddAsync(new WorkoutLog
        {
        ...
        Assert.False(exercise.IsLogged);
        Assert.Equal("Not logged yet", exercise.LogStatusText);
    }
