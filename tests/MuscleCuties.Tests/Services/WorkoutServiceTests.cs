...
public class WorkoutServiceTests
{
    ...
    [Fact]
    public async Task GenerateUserPlans_MenstrualRecoveryDays_ArePassive()
    {
        SetupEmptyExercises();
        _workoutRepo.GetAllUserPlansAsync(1).Returns(new List<WorkoutPlan>());
        _cycleService.GetCurrentPhaseAsync(1).Returns(CyclePhase.Follicular);
        ...
        await CreateService().GenerateUserPlansAsync(1, UserGoal.Strength, 3);

        var menstrual = capturedPlans.First(p => p.CyclePhaseTarget == CyclePhase.Menstrual);
        Assert.True(menstrual.WorkoutDays.All(d =>
            d.WorkoutType != WorkoutType.Recovery || d.RecoveryType == RecoveryType.PassiveRecovery));
    }

    [Fact]
    public async Task GenerateUserPlans_FollicularRecoveryDays_AreActive()
    {
        SetupEmptyExercises();
        _workoutRepo.GetAllUserPlansAsync(1).Returns(new List<WorkoutPlan>());
        _cycleService.GetCurrentPhaseAsync(1).Returns(CyclePhase.Follicular);
        ...
        await CreateService().GenerateUserPlansAsync(1, UserGoal.FatLoss, 5);

        var follicular = capturedPlans.First(p => p.CyclePhaseTarget == CyclePhase.Follicular);
        var recoveryDays = follicular.WorkoutDays.Where(d => d.WorkoutType == WorkoutType.Recovery).ToList();
        Assert.True(recoveryDays.All(d => d.RecoveryType == RecoveryType.ActiveRecovery));
    }

    [Fact]
    public async Task GenerateUserPlans_LutealStrengthRecoveryDays_ArePassive()
    {
        SetupEmptyExercises();
        _workoutRepo.GetAllUserPlansAsync(1).Returns(new List<WorkoutPlan>());
        _cycleService.GetCurrentPhaseAsync(1).Returns(CyclePhase.Follicular);
        ...
        await CreateService().GenerateUserPlansAsync(1, UserGoal.Strength, 5);

        var luteal = capturedPlans.First(p => p.CyclePhaseTarget == CyclePhase.Luteal);
        var recoveryDays = luteal.WorkoutDays.Where(d => d.WorkoutType == WorkoutType.Recovery).ToList();
        Assert.True(recoveryDays.All(d => d.RecoveryType == RecoveryType.PassiveRecovery));
    }

    [Fact]
    public async Task GenerateUserPlans_LutealFatLossRecoveryDays_AreActive()
    {
        SetupEmptyExercises();
        _workoutRepo.GetAllUserPlansAsync(1).Returns(new List<WorkoutPlan>());
        _cycleService.GetCurrentPhaseAsync(1).Returns(CyclePhase.Follicular);
    ...
    [Fact]
    public async Task GenerateUserPlans_NonRecoveryDays_HaveRecoveryTypeNone()
    {
        SetupEmptyExercises();
        _workoutRepo.GetAllUserPlansAsync(1).Returns(new List<WorkoutPlan>());
        _cycleService.GetCurrentPhaseAsync(1).Returns(CyclePhase.Follicular);
        ...
        var allDays = capturedPlans.SelectMany(p => p.WorkoutDays);
        Assert.True(allDays
            .Where(d => d.WorkoutType != WorkoutType.Recovery)
            .All(d => d.RecoveryType == RecoveryType.None));
    }
}
